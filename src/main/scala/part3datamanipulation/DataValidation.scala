...
object DataValidation {
  ...
  // acts like an either, but it combines errors
  ...
  // using Validated
  //
  // import cats.instances.list._ // appropriate semigroup for lists (not needed, the default is ol)
  // appropriate semigroup for the number (the default is the sum)
  ...
  validateNumber(2).fold(println, println)
  validateNumber(420).fold(println, println)

  //
  // API

  // chain (it is not flatMap because flatMap short circuit the error interrupting the chain,
  // there is no monad instance for Validated)
  ...
  object FormValidation {
    ...
    // import cats.instances.string._ use concat to combine strings, but we do not care
    // because we discard intermediate results if the validation has completed successfully
